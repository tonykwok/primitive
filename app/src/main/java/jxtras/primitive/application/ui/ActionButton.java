...
public class ActionButton extends JButton {
    ...
    private Image mainButton, mainButtonPressed, normalButton, normalButtonPressed,
            buttonHighlight;
    ...
    public ActionButton(String text, boolean main) {
        this();
        setText(text);
        setMain(main);
    }
    ...
    public ActionButton() {
        shadowColor = Color.decode("#000000");
        shadowDirection = 60;
        try {
            mainButton = GraphicsUtilities
                    .loadCompatibleImage(getClass().getResourceAsStream("/icon/button-main.png"));
            mainButtonPressed = GraphicsUtilities.loadCompatibleImage(
                    getClass().getResourceAsStream("/icon/button-main-pressed.png"));
            normalButton = GraphicsUtilities.loadCompatibleImage(
                    getClass().getResourceAsStream("/icon/button-normal.png"));
            normalButtonPressed = GraphicsUtilities.loadCompatibleImage(
                    getClass().getResourceAsStream("/icon/button-normal-pressed.png"));
            buttonHighlight = GraphicsUtilities
                    .loadCompatibleImage(getClass().getResourceAsStream("/icon/header-halo.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        shadowDistance = 1;
        sourceInsets = new Insets(6, 7, 6, 8);
        buttonDimension = new Dimension(150, 35);
        buttonForeground = Color.decode("#FFFFFF");
        buttonFont = new Font("San Francisco Text", Font.PLAIN, 19);
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (ghostValue > 0.0f) {
            ...
            g2.drawImage(buttonHighlight, insets.left + 2, insets.top + 2, width - 4, height - 4,
                    null);
...
//                timer.stop();
//            }
//            timer = new Timer(300, new AnimateGhost(true));
//            timer.start();
        }
...
//                timer.stop();
//            }
//            timer = new Timer(300, new AnimateGhost(false));
//            timer.start();
        }
    }

//    private final class AnimateGhost implements ActionListener {
